  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      ...
      try {
        ...
        // Fetch project data directly first
        ...
        if (projectError) {
          ...
          // Try RPC as fallback
          const { data: project, error: rpcError } = await supabase.rpc('handle_project_update', {
        ...
        if (propertyFnError) {
          ...
          // Fallback to direct query with additional error handling
          try {
