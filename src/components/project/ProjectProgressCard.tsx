...
const ProjectProgressCard = ({
  ...
  const determineStage = (): {
    ...
    if (allWorkBlocksComplete) return {
      stage: "finalization",
      progress: 100
    };
    ...
    if (hasScheduledWorkBlocks) return {
      stage: "building",
      progress: 80
    };
      ...
      <CardContent className="p-4">
        ...
        <ScrollArea className="w-full overflow-x-auto mb-4">
          <div className="flex items-center justify-between min-w-[660px] pb-2">
            ...
            return <React.Fragment key={stageKey}>
                  <div className={cn("flex items-center justify-center px-3 py-2 rounded-full transition-all whitespace-nowrap", isActive ? "bg-[#0e475d] text-white" : isPast ? "bg-[#0e475d] text-white" : "bg-white border border-gray-300 text-gray-600")}>
                    <span className="mr-1">{value.icon}</span>
                    <span className="text-sm font-medium">{value.name}</span>
                  </div>
                  
                  {/* Connector line */}
                  {index < array.length - 1 && <div className={cn("h-[2px] w-6", isPast || isActive ? "bg-[#0e475d]" : "bg-gray-300")} />}
                </React.Fragment>;
          })}
          </div>
        </ScrollArea>
