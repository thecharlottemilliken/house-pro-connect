...
import { TagsMetadata, TagDefinition, TagCategory, getTagDefinition, createTagId } from "@/utils/assetTagUtils";
import { ScrollArea } from "@/components/ui/scroll-area";

interface EnhancedTagsDialogProps {
  open: boolean;
...
const EnhancedTagsDialog: React.FC<EnhancedTagsDialogProps> = ({
  open,
  onOpenChange,
  selectedTags = [],
  onSaveTags,
  ...
  const [workingTags, setWorkingTags] = useState<string[]>(selectedTags);
  const [newTagInput, setNewTagInput] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const [activeCategory, setActiveCategory] = useState<TagCategory | 'all'>('all');

  // Reset working tags when dialog opens with new selected tags
  useEffect(() => {
    if (open) {
      setWorkingTags(selectedTags);
      setNewTagInput('');
      setSearchQuery('');
    }
  }, [open, selectedTags]);
  ...
  const availableTags = useMemo(() => {
    if (!tagsMetadata) return [];
    
    return Object.values(tagsMetadata.tags)
      .filter(tag => {
  ...
  }, [tagsMetadata, searchQuery, activeCategory]);

  // Get tag definitions for currently selected tags
  const selectedTagDefinitions = useMemo(() => {
    return workingTags.map(tagId => getTagDefinition(tagId, tagsMetadata));
  }, [workingTags, tagsMetadata]);
          ...
          <Button 
            onClick={() => {
              ...
              if (assetType && !workingTags.some(tag => tag.startsWith('type:'))) {
                onSaveTags([...workingTags, `type:${assetType}`]);
              } else {
                onSaveTags(workingTags);
              }
              onOpenChange(false);
            }}
          >
...
export default EnhancedTagsDialog;
